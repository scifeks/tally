...
class ToolOnRepoScan(ScanType):
    ...
    def execute(
        ...
        repo = next(
            (r for r in repos if r.name.lower() == self.repo_name.lower()),
            None,
        )
        if repo is None:
            raise ValueError(
        ...
        resources.event_sink.emit(
            se.ToolStarted(
                ...
                repo=repo.name,
                tool=self.tool_name,
                message=f"{self.tool_name} on {repo.name} started",
            )
        )
        ...
        if result is None:
            ...
            resources.event_sink.emit(
                se.ToolSkipped(
                    ...
                    repo=repo.name,
                    tool=self.tool_name,
                    message=f"{self.tool_name} skipped: no result",
            ...
            if result.success:
                ...
                resources.event_sink.emit(
                    se.ToolFailed(
                        ...
                        repo=repo.name,
                        tool=self.tool_name,
                        message=f"{self.tool_name} on {repo.name} failed",
                        exit_code=1,
                        duration=result.duration_seconds,
                    )
                )
