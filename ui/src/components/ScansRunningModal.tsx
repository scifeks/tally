      ...
      {running.length === 0 ? (
        ...
        <div className="space-y-3">
          <div className="text-[11px] text-muted-foreground leading-relaxed">
            <span className="text-dim">//</span> streaming segment progress via
            websocket. long-running ingestion + enrichment phases are normal.
          </div>
          {running.map((s) => {
...
function ProgressBar({ value }: { value: number }) {
  ...
  return (
    <div className="relative h-2 w-full border border-border bg-muted overflow-hidden">
      <div
        className={cn(
          "h-full bg-accent transition-[width] duration-500 ease-linear",
        )}
        style={{ width: `${pct}%` }}
      />
