  ...
  const generateEvents = useCallback((): { events: TriageLogEvent[]; batches: BatchDisplay[] } => {
    ...
    for (const segment of SEGMENTS) {
      ...
      for (let i = 0; i < segmentFindings.length; i += BATCH_SIZE) {
        ...
        addEvent(
          'batch_created',
          `Batch ${batchId} created: ${batchFindings.length} ${SEGMENT_LABEL[segment]} findings`,
          {
            batchId,
            segment,
    ...
    for (const batch of batchList) {
      addEvent('batch_started', `Processing ${batch.id}...`, {
        batchId: batch.id,
      ...
      for (let p = 0; p < 3; p++) {
        ...
        addEvent(
          'batch_progress',
          `Claude analyzing... ${Math.min(100, Math.floor((p + 1) * 33))}%`,
          {
            batchId: batch.id,
      ...
      if (fails) {
        addEvent('batch_failed', `${batch.id} failed: API timeout`, {
          batchId: batch.id,
          segment: batch.segment,
        })
        addEvent('batch_retry', `Retrying ${batch.id} (attempt 2)...`, {
          batchId: batch.id,
          attempt: 2,
        })
        // Retry succeeds
        addEvent(
          'batch_completed',
          `${batch.id} complete (retry): ${batch.findingCount} findings triaged`,
          {
            batchId: batch.id,
            segment: batch.segment,
        ...
        addEvent(
          'batch_completed',
          `${batch.id} complete: ${batch.findingCount} findings triaged in ${thinkTime.toFixed(1)}s`,
          {
            batchId: batch.id,
            segment: batch.segment,
            findingsCount: batch.findingCount,
    ...
    addEvent(
      'run_completed',
      `Triage complete: ${eligibleFindings.length} findings processed across ${batchList.length} batches`
    )
    ...
    eventIntervalRef.current = setInterval(() => {
      ...
      if (next.type === 'batch_started' && next.batchId) {
        setBatches(prev =>
          prev.map(b =>
            b.id === next.batchId
              ? { ...b, status: 'in_progress' as const, startedAt: next.timestamp }
              : b
          )
        )
        ...
        setBatches(prev =>
          prev.map(b =>
            b.id === next.batchId
              ? { ...b, status: 'completed' as const, finishedAt: next.timestamp }
              : b
          )
        )
        batchIdx++
        ...
        setBatches(prev =>
          prev.map(b =>
            b.id === next.batchId
              ? { ...b, status: 'in_progress' as const, attempt: next.attempt ?? 2 }
              : b
          )
        )
      }
