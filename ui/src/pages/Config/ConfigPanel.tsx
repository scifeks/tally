...
export function ConfigPanel({ active, size = 180 }: { active?: boolean; size?: number }) {
  const gearConfigs = [
    ...
    { cx: size * 0.35, cy: size * 0.7, r: size * 0.1, teeth: 6, speed: 10 },
  ]
  ...
  return (
    ...
    >
      ...
      {gearConfigs.map((gear, idx) => (
        <g
          ...
          style={{
            ...
            animation: active
              ? `spin ${Math.abs(gear.speed)}s linear infinite ${gear.speed < 0 ? 'reverse' : 'normal'}`
              : undefined,
          }}
        >
          <path
      ...
      {active && (
        <g>
          {[0, 1, 2].map(i => (
            <circle key={i} r={2} fill="var(--color-accent)" opacity={0.8} className="tty-glow">
              <animateMotion
                dur={`${2 + i * 0.5}s`}
