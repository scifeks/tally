...
class ZAPLocalTool(BaseZapTool):
    ...
    **OpenAPI mode:**
    ...
    def build_execution_passes(self, context: ExecutionContext) -> list[ExecutionPass]:
        ...
        Phase 9: ZAP receives an OpenAPI spec via ``openapi_file`` whenever
        the target repo has any rows in ``url_findings`` — the merged OAS3
        document is JIT-rebuilt from the DB right before the scan runs.
        Falls back to ``-quickurl`` mode when no rows exist.
        """
