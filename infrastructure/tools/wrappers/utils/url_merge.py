...
URL discovery now writes to the ``url_findings`` table and the artifact
builder rebuilds ``merged_urls.txt`` / ``merged_oas3.json`` from those rows
JIT. ``_normalise_url`` is preserved here because the artifact builder
...
def _normalise_url(url: str) -> str:
    ...
    - Ignore scheme differences (http vs https); compare by
      ``host:port/path``.
    """
    try:
        parsed = urlsplit(url)
