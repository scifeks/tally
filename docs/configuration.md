...
**Created:** When the first repository is added via `add-repo`.
...
| `path` | string | yes | Absolute filesystem path to the repository. Must exist on disk. |
...
**Created:** When `new-project` is run. Starts as `{}`.
