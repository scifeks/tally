...
Merge priority is Katana, then Noir, then user file. Deduplication preserves first-seen
...
`repositories.json` (`merged_seeds_path`, `merged_oas3_path`). Subsequent
tool runs can consume them without re-running discovery.
...
Headless mode (`katana_headless: true`) launches a real Chrome instance.
...
cyclic parameterized URLs (e.g. `?id=1`, `?id=2`, …). The crawler has a
...
credentials are configured, Tally falls back to unauthenticated crawling without
showing an error. If Katana returns far fewer endpoints than expected, check that
the env var is exported in the current shell.
...
[OWASP Noir](https://github.com/noir-cr/noir) analyzes source code and emits an
...
Noir is skipped in any of the following cases:
...
(`/a`, `/b`, `/0x`, …) instead of real routes. Tally reads the repository's
`dependencies_file` and skips Noir if an unsupported package is found.
...
apps by the presence of `package.json` at the repo root and skips Noir for them.
...
lock files and passes path-prefix exclusions to Noir. Vendor endpoints are
stripped before the OAS3 output is written.
...
4. **Deduplicate.** URLs are normalized (lowercase host, strip default ports,
   strip trailing slashes, scheme-insensitive) and deduplicated in order:
   Katana first, then Noir, then user file.
5. **Write outputs.**
   - `projects/<project>/endpoints/<repo>/merged_urls.txt` contains one URL per line,
     consumed by XSStrike and DalFox.
   - `projects/<project>/endpoints/<repo>/merged_oas3.json` is a minimal OAS3
...
6. **Persist.** `merged_seeds_path` and `merged_oas3_path` are written back to
   `repositories.json`. Subsequent tool runs can consume the outputs without
