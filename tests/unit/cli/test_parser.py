...
class TestParserSubcommands:
    ...
    def test_run_command_parses(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["run", "semgrep"])
    ...
    def test_ui_command_with_serve_parses(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["ui", "serve"])
        assert args.command == "ui"
        assert args.ui_command == "serve"

    def test_missing_required_command_raises_system_exit(self) -> None:
        parser = build_parser()
        with pytest.raises(SystemExit):
            parser.parse_args([])


class TestScanCommandDefaults:
...
class TestScanCommandToolMutualExclusion:
    def test_tool_and_skip_tools_are_mutually_exclusive(self) -> None:
        parser = build_parser()
        with pytest.raises(SystemExit):
            parser.parse_args(["scan", "--tool=semgrep", "--skip-tools=gitleaks"])

    def test_tool_alone_is_allowed(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["scan", "--tool=semgrep"])
        assert args.tool == "semgrep"

    def test_skip_tools_alone_is_allowed(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["scan", "--skip-tools=gitleaks"])
        assert args.skip_tools == "gitleaks"


class TestCommaSeparatedFlagsAreRawStrings:
    def test_comma_separated_tools_returned_as_string(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["scan", "--tool=semgrep,gitleaks,truffleHog"])
        assert args.tool == "semgrep,gitleaks,truffleHog"

    def test_comma_separated_repos_returned_as_string(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["scan", "--repo=repo1,repo2,repo3"])
        assert args.repo == "repo1,repo2,repo3"

    def test_comma_separated_domains_returned_as_string(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["scan", "--domain=api.example.com,www.example.com"])
        assert args.domain == "api.example.com,www.example.com"

    def test_comma_separated_skip_tools_returned_as_string(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["scan", "--skip-tools=semgrep,gitleaks"])
        assert args.skip_tools == "semgrep,gitleaks"


class TestRunCommand:
    def test_run_with_tool_only(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["run", "semgrep"])
        assert args.command == "run"
        assert args.tool == "semgrep"
    ...
    def test_run_with_timeout(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["run", "--timeout=300", "semgrep"])
        assert args.timeout == 300

    def test_run_with_remainder_args(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["run", "semgrep", "--", "--verbose"])
        assert args.tool == "semgrep"
    ...
    def test_run_with_timeout_and_remainder(self) -> None:
        ...
        args = parser.parse_args(
            ["run", "--timeout=300", "semgrep", "--", "--verbose", "arg2"]
        )
        assert args.timeout == 300
        assert args.args == ["--verbose", "arg2"]
...
class TestReportCommand:
    def test_report_with_no_subcommand(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["report"])
        assert args.command == "report"
        assert args.report_command is None
    ...
    def test_report_testing_type_defaults_to_white_box(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["report"])
        assert args.testing_type == "white_box"

    def test_report_with_draft_subcommand(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["report", "draft"])
        assert args.command == "report"
        assert args.report_command == "draft"

    def test_report_draft_with_section(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["report", "draft", "executive_summary"])
        assert args.report_command == "draft"
        assert args.section == "executive_summary"

    def test_report_draft_force_flag(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["report", "draft", "--force"])
        assert args.force is True

    def test_report_draft_skip_triage_flag(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["report", "draft", "--skip-triage"])
        assert args.skip_triage is True

    def test_report_shell_subcommand(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["report", "shell"])
        assert args.command == "report"
        assert args.report_command == "shell"

    def test_report_shell_with_testing_type(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["report", "shell", "--testing-type=black_box"])
        assert args.testing_type == "black_box"

    def test_report_shell_with_engagement_date(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["report", "shell", "--engagement-date=2025-05-10"])
        assert args.engagement_date == "2025-05-10"

    def test_report_shell_with_output(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["report", "shell", "--output=/tmp/report.pdf"])
...
class TestTriageCommand:
    def test_triage_batch_flag(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["triage", "--batch"])
    ...
    def test_triage_rebuild_container_flag(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["triage", "--rebuild-container"])
        assert args.rebuild_container is True

    def test_triage_all_flags_together(self) -> None:
        parser = build_parser()
        args = parser.parse_args(
            ["triage", "--batch", "--dry-run", "--rebuild-container"]
        )
        assert args.batch is True
        assert args.dry_run is True
        assert args.rebuild_container is True
...
class TestPurgeCommand:
    def test_purge_with_tool(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["purge", "--tool=semgrep"])
        assert args.tool == "semgrep"

    def test_purge_with_multiple_tools(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["purge", "--tool=semgrep,gitleaks"])
        assert args.tool == "semgrep,gitleaks"

    def test_purge_keep_reports_flag(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["purge", "--keep-reports"])
        assert args.keep_reports is True


class TestGlobalFlags:
    def test_base_path_defaults_to_current_directory(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["stats"])
    ...
    def test_base_path_can_be_overridden(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["--base-path=/tmp", "stats"])
    ...
    def test_project_can_be_set(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["--project=my_project", "stats"])
        assert args.project == "my_project"

    def test_global_flags_before_command(self) -> None:
        parser = build_parser()
        args = parser.parse_args(["--base-path=/tmp", "--project=proj", "stats"])
        assert args.base_path == "/tmp"
