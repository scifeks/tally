"""Unit tests for get_tool_version (domain.tools.base)."""
...
def test_returns_none_when_binary_not_on_path() -> None:
    with patch("domain.tools.base.shutil.which", return_value=None):
...
def test_returns_first_line_of_stdout() -> None:
    ...
    with (
        patch("domain.tools.base.shutil.which", return_value="/usr/bin/mytool"),
        patch("domain.tools.base.subprocess.run", return_value=proc),
    ):
...
def test_returns_none_on_file_not_found_error() -> None:
    with (
        patch("domain.tools.base.shutil.which", return_value="/usr/bin/mytool"),
        patch(
            "domain.tools.base.subprocess.run",
...
def test_returns_none_on_os_error() -> None:
    with (
        patch("domain.tools.base.shutil.which", return_value="/usr/bin/mytool"),
        patch("domain.tools.base.subprocess.run", side_effect=OSError),
    ):
        assert get_tool_version("mytool") is None
