"""Unit tests for XSStrike katana seeds helpers and katana/auto modes."""
...
def _write_katana_oas3(katana_dir: Path, repo_name: str, paths: list[str]) -> Path:
    """Write a Katana OAS3 fixture file."""
...
def _write_noir_oas3(noir_dir: Path, repo_name: str, paths: list[str]) -> Path:
    """Write a Noir OAS3 fixture file."""
...
class TestBuildSeedsFromKatana:
    def test_happy_path_returns_seeds_file(self, tmp_path: Path) -> None:
        ...
        _write_katana_oas3(katana_dir, "repo", ["/api/users", "/api/items"])
    ...
    def test_seeds_file_contains_full_urls(self, tmp_path: Path) -> None:
        katana_dir = tmp_path / "projects" / "proj" / "tool_outputs" / "katana"
    ...
    def test_missing_katana_dir_returns_none(self, tmp_path: Path) -> None:
        result = _build_seeds_from_katana(
            str(tmp_path),
            "proj",
            "repo",
