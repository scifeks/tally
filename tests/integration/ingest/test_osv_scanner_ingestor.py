...
class TestOsvScannerIngestor:
    ...
    def test_shared_metadata(self, osv_parsed_data: dict) -> None:
        handler = ToolHandlerFactory.load("osv-scanner")
    ...
    def test_cvss_score_present(self, osv_parsed_data: dict) -> None:
        handler = ToolHandlerFactory.load("osv-scanner")
        assert handler is not None
        result = _make_sca_result("osv-scanner", osv_parsed_data)
        rows = handler.normalize(result, profile="test-repo")
        assert rows
        row = rows[0]
        assert "cvss_score" in row
        assert isinstance(row["cvss_score"], float)

    def test_lockfile_present(self, osv_parsed_data: dict) -> None:
        handler = ToolHandlerFactory.load("osv-scanner")
        assert handler is not None
        result = _make_sca_result("osv-scanner", osv_parsed_data)
        rows = handler.normalize(result, profile="test-repo")
        assert rows
        row = rows[0]
        assert "lockfile" in row
        assert row["lockfile"] == "requirements.txt"

    def test_return_type_is_list(self, osv_parsed_data: dict) -> None:
        handler = ToolHandlerFactory.load("osv-scanner")
        assert handler is not None
        result = _make_sca_result("osv-scanner", osv_parsed_data)
        rows = handler.normalize(result, profile="test-repo")
        assert isinstance(rows, list)
        assert len(rows) == len(osv_parsed_data["vulnerabilities"])
