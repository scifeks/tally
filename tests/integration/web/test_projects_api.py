...
class TestListProjectsV1:
    async def test_list_projects_returns_active(self, projects_v1_client) -> None:
        client, _ = projects_v1_client
        resp = await client.get("/api/v1/projects/")
        assert resp.status_code == 200
        items = resp.json()["items"]
        names = [i["id"] for i in items]
        assert "testproject" in names
        active = next(i for i in items if i["id"] == "testproject")
...
class TestProjectMetaV1:
    async def test_project_meta_returns_counts(self, projects_v1_client) -> None:
        client, _ = projects_v1_client
        resp = await client.get("/api/v1/projects/testproject/meta")
        assert resp.status_code == 200
        data = resp.json()
    ...
    async def test_project_meta_unknown_returns_404(self, projects_v1_client) -> None:
        client, _ = projects_v1_client
        resp = await client.get("/api/v1/projects/nonexistent/meta")
...
class TestProjectInfoV1:
    async def test_project_info_returns_full_fields(self, projects_v1_client) -> None:
        client, _ = projects_v1_client
        resp = await client.get("/api/v1/projects/testproject/info")
        assert resp.status_code == 200
        data = resp.json()
        for field in (
            "company",
    ...
    async def test_project_info_unknown_returns_404(self, projects_v1_client) -> None:
        client, _ = projects_v1_client
        resp = await client.get("/api/v1/projects/nonexistent/info")
        assert resp.status_code == 404
...
class TestRepositoriesV1:
    async def test_repositories_list_omits_auth(self, projects_v1_client) -> None:
        client, _ = projects_v1_client
        resp = await client.get("/api/v1/projects/testproject/repositories")
        assert resp.status_code == 200
        raw = resp.text
    ...
    async def test_repositories_list_pagination(self, projects_v1_client) -> None:
        client, _ = projects_v1_client
        resp = await client.get("/api/v1/projects/testproject/repositories?limit=1")
        assert resp.status_code == 200
        data = resp.json()
        assert data["total"] >= 1
        assert len(data["items"]) == 1
    ...
    async def test_repositories_list_unknown_project_returns_404(
        ...
        client, _ = projects_v1_client
        resp = await client.get("/api/v1/projects/nonexistent/repositories")
        assert resp.status_code == 404
