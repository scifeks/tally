...
class PipAuditWrapper(ToolWrapper):
    ...
    def build_command(self, **kwargs) -> List[str]:
        ...
        elif (repo / "pyproject.toml").exists() or (repo / "setup.cfg").exists() or (repo / "setup.py").exists():
            cmd.extend(["--path", str(repo)])
        else:
            raise ValueError(
                f"No Python manifest (requirements.txt, pyproject.toml, setup.py) found in {repo_path!r}"
            )
