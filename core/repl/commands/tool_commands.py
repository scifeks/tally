...
class ToolCommands:
    ...
    def _cmd_tool_list(self) -> None:
        from core.tools.registry import tool_registry, build_tool_table
        tools = tool_registry.get_all_tools()
        if not tools:
            self.repl.console.print(
                '[dim]No tools configured. Use [bold]tool add[/bold] to add a tool.[/dim]'
            )
            return
    ...
    def _cmd_tool_add(self) -> None:
        ...
        if not unconfigured:
            self.repl.console.print('[dim]All available tools are already configured.[/dim]')
            return
    ...
    def _cmd_tool_edit(self, tool_name: str) -> None:
        ...
        entry = interview_tool(tool_name, has_local, has_docker, defaults=commands[tool_name])
    ...
    def _cmd_tool_remove(self, tool_name: str) -> None:
        ...
        raw = input(
            f"  Remove tool '{tool_name}'? This cannot be undone. [y/N]: "
        ).strip().lower()
        if raw not in ('y', 'yes'):
            self.repl.console.print('Cancelled.')
            return
