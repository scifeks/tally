...
class ReportGenerator:
    ...
    def generate(
        ...
        if output_format == 'markdown':
            ...
            raise ValueError(f"Unknown format: {output_format!r}. Use markdown, html, or json.")
    ...
    def _render_markdown(self, aggregated: Dict[str, Any]) -> str:
            ...
            if sca_findings:
                ...
                for f in sca_findings:
                    lines.append(
                        ...
                        f"| {f.get('package_version', '')} | {f.get('vulnerability_id', '')} "
        ...
        if gitleaks_findings:
            ...
            lines.append(
                "_Note: Secret values are never stored or displayed._"
            )
    ...
    def _render_html(self, aggregated: Dict[str, Any]) -> str:
        """Render aggregated findings as a self-contained HTML report (inline CSS only)."""
        project_name = aggregated['project_name']
        generated_at = aggregated['generated_at']
        summary = aggregated['summary']
        findings = aggregated['findings']
        ...
        css = (
            ...
            ".badge{padding:6px 14px;border-radius:6px;color:#fff;font-weight:bold;font-size:.9em;}"
            ...
            ".section{background:#fff;border-radius:8px;padding:16px 24px;margin-bottom:20px;"
            "box-shadow:0 1px 3px rgba(0,0,0,.1);}"
            ...
            ".sev{padding:2px 6px;border-radius:3px;font-size:.85em;color:#fff;font-weight:bold;}"
        ...
        def sev_badge(sev: str) -> str:
            cls = f"sev sev-{sev.lower()}" if sev.lower() in ('critical', 'high', 'medium', 'low') else "sev"
        ...
        if nmap_ports:
            rows = "".join(
                tr(f.get('ip_address', ''), str(f.get('port', '')),
                   f.get('service', ''), 'tcp')
                for f in nmap_ports
            )
            sections.append(
        ...
        if semgrep_findings:
            rows = "".join(
                tr(sev_badge(f.get('severity', '')), f.get('rule_id', ''),
                   f.get('file_path', ''), str(f.get('line_start', '')),
                   f.get('cwe', ''))
                for f in semgrep_findings
            )
            sections.append(
        ...
        for tool in _SCA_TOOLS:
            ...
            if sca_findings:
                rows = "".join(
                    tr(sev_badge(f.get('severity', '')), f.get('package_name', ''),
                       f.get('package_version', ''), f.get('vulnerability_id', ''),
                       f.get('fixed_version', ''))
                    for f in sca_findings
                )
                sections.append(
                    f'<div class="section"><h3>SCA ({tool})</h3>'
                    f'<table>{th("Severity", "Package", "Version", "Vulnerability ID", "Fixed In")}'
                    f'<tbody>{rows}</tbody></table></div>'
                )
        ...
        if gitleaks_findings:
            rows = "".join(
                tr(f.get('rule_id', ''), f.get('file_path', ''), str(f.get('line_number', '')))
                for f in gitleaks_findings
            )
            sections.append(
                ...
                '<p class="note">Note: Secret values are never stored or displayed.</p></div>'
            )
        ...
        if zap_findings:
            rows = "".join(
                tr(sev_badge(f.get('severity', '')), f.get('alert_name', ''),
                   f.get('url', ''), f.get('method', ''),
                   f.get('param', ''), str(f.get('cwe_id', '')))
                for f in zap_findings
            )
            sections.append(
